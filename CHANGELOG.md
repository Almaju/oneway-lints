...
### Fixed
- **`raw_primitive_param` no longer fires on trait impl method signatures.** Trait impls (`impl FromStr for X { fn from_str(s: &str) -> ... }`) have their parameter shape fixed by the trait declaration — the lint can't ask the user to change `s: &str` to a newtype because the trait wouldn't accept it. Now tracks `in_trait_impl_depth` via `check_item` / `check_item_post` and skips trait impl method bodies entirely (same approach `subject_first_param` already uses). If you own the trait and the parameter shape is wrong, fix it at the trait declaration.
