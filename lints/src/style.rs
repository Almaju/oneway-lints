...
trait SrcExt {
    fn find_comments(&self) -> Vec<(usize, usize)>;
    fn group_comments(&self, comments: &[(usize, usize)]) -> Vec<Vec<(usize, usize)>>;
    fn is_block_comment_at(&self, lo: usize) -> bool;
    fn is_justified_comment_line(&self) -> bool;
}
...
impl SrcExt for str {
    ...
    fn is_block_comment_at(&self, lo: usize) -> bool {
        self.as_bytes().get(lo..lo + 2) == Some(b"/*")
    }

    /// Returns true if this line of comment content carries a label, link,
    /// or ticket reference that justifies the comment's existence.
    fn is_justified_comment_line(&self) -> bool {
        let trimmed = self.trim_start_matches(['/', '*']).trim_start();
...
impl EarlyLintPass for NoComments {
    fn check_crate(&mut self, early_context: &EarlyContext<'_>, _krate: &ast::Crate) {
        ...
        source_map.files().iter().for_each(|file| {
            ...
            let comments = src.find_comments();
            src.group_comments(&comments).into_iter().for_each(|group| {
                let parts: Vec<&str> = group.iter().map(|&(lo, hi)| &src[lo..hi]).collect();
                let group_text = parts.join("\n");
                if group_text.lines().any(str::is_justified_comment_line) {
                    return;
                }
